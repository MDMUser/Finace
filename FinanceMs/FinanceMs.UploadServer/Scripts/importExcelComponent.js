...
SWFUpload.prototype.initSWFUpload = function (b) {
    try { this.customSettings = {}; this.settings = b; this.eventQueue = []; this.movieName = "SWFUpload_" + SWFUpload.movieCount++; this.movieElement = null; SWFUpload.instances[this.movieName] = this; this.initSettings(); this.loadFlash(); this.displayDebugInfo() } catch (a) { delete SWFUpload.instances[this.movieName]; throw a }
...
/*错误信息封装对话框*/
var message = (function (message, easyMsg) {
    var def = $.fn.window.defaults;
    easyMsg.defaults.ok = "确定";
    easyMsg.defaults.cancel = "取消";
    message.error = function (options, call) {
        if (typeof options == 'object') {
            easyMsg.alert(options.title || '　错误', options.message, call || options.callback);
        }
        else {
            easyMsg.alert('　错误', options, call);
        }
    }

    message.progress = function (options, call) {
    }
    return message;
})(message || {}, $.messager);
/**
* MDM FinaceExcelImport
* 
**/
jQuery.FinaceExcelImport = function (param, callBack) {
    var version = "v0.1.1";
    var buildDom = {
        ids: function () {
            return {
        ...
        dialog: function () {
            var ids = this.ids();
            return "<div id='" + ids.dialogID + "' class='webImp'>"
                + "<input type='file' name='uploadify' id='" + ids.uploadify + "'/>"
                + "</div>";
        }
    };
    ...
    function createDialog(param) {
        var ids = buildDom.ids(),
            dialog = buildDom.dialog();

        if ($('#' + ids.dialogID).length == 0) {
            ...
            $('#' + ids.dialogID).dialog({
                ...
                buttons: [{
                    ...
                    text: '取消',
                    handler: function () {
                        $('#' + ids.dialogID).dialog('close');
                        //  $('#' + ids.dialogID).dialog('destroy');
                    }
                }],
                onClose: function () {
                    $('#' + ids.dialogID).dialog('destroy');
            ...
            $('#' + ids.dialogID).dialog('open');
        }
    }

    //绑定事件
    function bindEvents(param) {
        binduploadifyEvents(param);
    }

    //绑定选择文件后的预览事件
    function binduploadifyEvents(param) {
        var ids = buildDom.ids();
        var uploadOptions = {
            height: 24,
            width: 100,
            ...
            buttonText: $(this).attr('tagName') || '选择文件...',
            buttonCursor: 'hand',                // 按钮的鼠标图标
            fileSizeLimit: '0',
            preventCaching: true,              //阻止缓存
            auto: false,                      // 选择之后，自动开始上传
            fileTypeDesc: 'Excel Files',
            fileTypeExts: '*.xlsx;*.xlsm', //'*.xls;*.xlsx;*.xlsm',
            swf: '/cwbase/web/scripts/MDM/Finace/uploadify.swf?var=' + (new Date()).getTime(),
            uploader: '/cwbase/MDMWeb/FinaceImporter.ashx',
            ...
            onUploadSuccess: function (file, data, response) {
                ...
                if (result.result == "0") {
                    ...
                    vsMsg = vsMsg.replace(/%2c/g, ',');
                    vsMsg = vsMsg.replace(/%3b/g, ';');
                    vsMsg = vsMsg.replace(/%2f/g, '/');
                    var str = decodeURI(vsMsg);

                    // 弹出提示消息
                    $('<div id="dlgId"><div id="dlgContent"></div></div>').appendTo($('body'))
            ...
            onSelectError: function (file, errorCode, errorMsg) {
                switch (errorCode) {
                    case -100:
                        $.notify.info("上传的文件数量已经超出系统限制的" + $("#" + ids.uploadify).uploadify('settings', 'queueSizeLimit') + "个文件！");
                        break;
                    case -110:
                        $.notify.info("文件 [" + file.name + "] 大小超出系统限制的" + $("#" + ids.uploadify).uploadify('settings', 'fileSizeLimit') + "大小！");
                        break;
                    case -120:
                        $.notify.info("文件 [" + file.name + "] 的大小不能为0字节！");
                        break;
                    case -130:
                        $.notify.info("文件 [" + file.name + "] 类型不正确！");
                        break;
                }
                return false;
            },
            ...
            onFallback: function () {
                $.notify.info("没有安装flash player,请先安装flash player然后重启浏览器");
            },
            onCancel: function () {
                $.notify.success('取消上传成功！');
            }
        };

        setTimeout(function () {
            $("#" + ids.uploadify).uploadify(uploadOptions);
        }, 100);
    }
};