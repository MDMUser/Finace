    ...
    public class ZGBMOperate
    {
        ...
        public string ImportData(DataSet data)
        {
            ...
            for (int tableCount = 0; tableCount < data.Tables.Count; tableCount++)
            {
                ...
                // ①判断excel表格的合理性
                string[] list = { "主管部门代码", "主管部门名称", "级数", "是否明细" };
                msg = Verification.ImportColumns(data.Tables[0].Columns, list);
                if (!string.IsNullOrWhiteSpace(msg))
                    return msg;

                // ②将dataTable转为list
                var csList = ConvertsData.DataTableToListByProperties<MDMZGBM>(dtData);

                // ③筛选出添加、修改、无效数据
                // 声明(添加、修改)和 无效list
                IList<MDMZGBM> editList = new List<MDMZGBM>(),
                            invalidList = new List<MDMZGBM>();
        ...
        /// <returns></returns>
        private string EditListOperate(IList<MDMZGBM> cList)
        {
            string result = "";
            for (int i = 0; i < cList.Count; i++)
            {
                MDMZGBM addInfo = cList[i];
                var resModel = DBUtility.GetNewFJMByDict(db, "MDMZGBM", addInfo.Code, addInfo.Layer, addInfo.ParentCode);
                if (resModel != null && (!string.IsNullOrWhiteSpace(resModel.NewFJM) || !string.IsNullOrWhiteSpace(resModel.NM)))
                {
                    if (!string.IsNullOrWhiteSpace(resModel.NM))
                    {
                        ...
                        sqledit.AppendFormat("UPDATE MDMZGBM SET Name='{0}', IsDetail='{1}', ", addInfo.Name, addInfo.IsDetail);
        ...
        #region 无效数据处理
        private string InvalidOperate(IList<MDMZGBM> cList)
        {
            string info = "";
            for (int i = 0; i < cList.Count; i++)
            {
        ...
        public DataSet ExportData(string where)
        {
            ...
            sb.AppendLine(" SELECT a.Code 主管部门代码, a.Name 主管部门名称, a.Address 部门地址, a.Phone 部门联系电话, b.Name 上级部门,  ");
            sb.AppendLine(" a.Note 备注, a.Layer 级数, a.IsDetail 是否明细 ");
            sb.AppendLine(" FROM MDMZGBM a LEFT JOIN MDMZGBM b ON a.ParentNM=b.NM  WHERE a.TYBZ='0' and a.AuditState='2'");
            sb.AppendLine(where);
